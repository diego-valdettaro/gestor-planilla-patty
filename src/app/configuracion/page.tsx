...
export default async function PaginaDeConfiguracion({ searchParams }: { searchParams: Promise<{ grupo?: string; inactivos?: string }> }) {
  ...
  const grupoPorSede = new Map(listaDeSedes.map((sede) => [sede.nombre, sede.grupo]));
  ...
  return <main className="contenido pagina">
    ...
    {actor.rol === "administracion" && <><div className="grilla-configuracion">
      <section className="tarjeta panel"><header className="panel-cabecera"><div><h2>Grupos y sedes</h2><p>Cada sede pertenece a un grupo.</p></div></header><CreadorDeGrupo /><form action={guardarSede} className="filtros panel-filtros"><label>Nombre<input name="nombre" placeholder="Nueva sede" required /></label><label>Grupo<select defaultValue="" name="grupo" required><option disabled value="">Grupo…</option>{grupos.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><button className="boton-principal" type="submit">Crear sede</button></form><ul className="lista-configuracion lista-sedes">{listaDeSedes.map((sede) => <li key={sede.nombre}><span className="principal-configuracion"><strong>{sede.nombre}</strong><small>Grupo: {sede.grupo ?? "Sin asignar"}</small></span><span className="acciones-configuracion"><EditorDeGrupoDeSede grupoActual={sede.grupo} grupos={grupos} nombre={sede.nombre} /><BotonDeAccionConfirmada accion={eliminarSede} confirmar="Eliminar sede" descripcion="Solo puede eliminar una sede sin colaboradores asignados." etiqueta="Eliminar" peligro titulo={`¿Eliminar ${sede.nombre}?`}><input name="nombre" type="hidden" value={sede.nombre} /></BotonDeAccionConfirmada></span></li>)}</ul></section>
