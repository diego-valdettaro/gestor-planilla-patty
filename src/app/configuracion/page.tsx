...
export default async function PaginaDeConfiguracion({ searchParams }: { searchParams: Promise<{ grupo?: string; inactivos?: string }> }) {
  ...
  return <main className="contenido configuracion">
    ...
    {actor.rol === "administracion" && <><div className="grilla-configuracion">
      <section className="tarjeta tarjeta-configuracion"><header className="encabezado-seccion-configuracion"><div><h2>Sedes</h2><p>Cada sede pertenece a un grupo.</p></div></header><form action={guardarSede} className="filtros filtros-configuracion"><label>Nombre<input name="nombre" placeholder="Nueva sede" required /></label><label>Grupo<select defaultValue="" name="equipoOperativo" required><option disabled value="">Grupo…</option><option value="tiendas">Tiendas</option><option value="taller">Taller</option></select></label><button className="boton-principal" type="submit">Crear sede</button></form><ul className="lista-configuracion lista-sedes">{listaDeSedes.map((sede) => <li key={sede.nombre}><span className="principal-configuracion"><strong>{sede.nombre}</strong><small>Grupo: {etiquetaDeEquipo(sede.equipoOperativo)}</small></span><span className="acciones-configuracion"><details className="edicion-configuracion"><summary>Editar</summary><form action={asignarEquipoOperativoASede} className="formulario-edicion formulario-edicion-corto"><input name="nombre" type="hidden" value={sede.nombre} /><label>Grupo<select defaultValue={sede.equipoOperativo ?? ""} name="equipoOperativo" required><option disabled value="">Sin asignar</option><option value="tiendas">Tiendas</option><option value="taller">Taller</option></select></label><button className="boton-principal" type="submit">Guardar</button></form></details><BotonDeAccionConfirmada accion={eliminarSede} confirmar="Eliminar sede" descripcion="Solo puede eliminar una sede sin colaboradores asignados." etiqueta="Eliminar" peligro titulo={`¿Eliminar ${sede.nombre}?`}><input name="nombre" type="hidden" value={sede.nombre} /></BotonDeAccionConfirmada></span></li>)}</ul></section>
