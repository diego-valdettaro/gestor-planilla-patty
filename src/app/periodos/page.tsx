...
export default async function PaginaDePeriodos({ searchParams }: { searchParams: Promise<{ periodoId?: string; sede?: string; idHuellero?: string }> }) {
  ...
  return <main className="contenido">
    <header className="encabezado"><div><p className="eyebrow">Administración y Finanzas</p><h1>Resumen del período</h1><p>Filtre, revise y exporte los totales antes de cerrar el período.</p></div></header>
    {periodo ? <>
      ...
      <section className="tarjeta periodo-resumen">
        <header className="encabezado-seccion"><div><h2>Período {periodo.inicio} a {periodo.fin}</h2><p>Estado actual: <strong>{periodo.estado}</strong></p></div>{periodo.estado === "abierto" ? <BotonDeAccionConfirmada accion={cerrarPeriodoDesdeFormulario} confirmar="Cerrar período" descripcion="Ya no se podrán importar ni modificar asistencias de este período hasta que lo reabra con un motivo." etiqueta="Cerrar período" titulo="¿Cerrar este período de planilla?"><input type="hidden" name="periodoId" value={periodo.id} /></BotonDeAccionConfirmada> : null}</header>
        {periodo.estado === "cerrado" ? <form action={reabrirPeriodoDesdeFormulario} className="filtros"><input type="hidden" name="periodoId" value={periodo.id} /><label>Motivo de reapertura<input name="motivo" required /></label><button type="submit">Reabrir período</button></form> : null}
        {resumen.length ? <div className="tabla-semanal"><table><thead><tr><th>Colaborador</th><th>Sede</th><th>Horas trabajadas</th><th>Tardanzas</th><th>Saldo penalizado</th><th>Extras 25%</th><th>Extras 35%</th></tr></thead><tbody>{resumen.map((fila) => <tr key={fila.idHuellero}><td>{fila.nombre} ({fila.idHuellero})</td><td>{fila.sede}</td><td>{(fila.minutosTrabajados / 60).toFixed(2)}</td><td>{fila.cantidadTardanzas}</td><td>{(fila.minutosPenalizados / 60).toFixed(2)}</td><td>{(fila.minutosAl25 / 60).toFixed(2)}</td><td>{(fila.minutosAl35 / 60).toFixed(2)}</td></tr>)}</tbody></table></div> : <section className="estado-vacio"><h3>Sin resultados</h3><p>No hay asistencias que coincidan con los filtros elegidos.</p></section>}
      </section>
